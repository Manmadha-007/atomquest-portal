...
function getActivitySubject(log: AuditLogRecord) {
  return (
...
function getStatusCount(
  statusDistribution: Array<{ count: number; status: GoalStatus }>,
  status: GoalStatus,
) {
  ...
  return [
    {
      ...
      description: "Active-cycle objectives across the enterprise.",
      icon: Target,
    ...
    },
    {
      ...
      description: "Goal completion from quarterly progress signals.",
      icon: CheckCircle2,
    ...
    },
    {
      ...
      description: "Active employee population in the analytics scope.",
      icon: UsersRound,
...
function QuickActionCard({ action }: { action: QuickAction }) {
  ...
  return (
    <Link
      href={action.href}
      className="group rounded-lg border bg-background p-4 transition-colors hover:border-primary/40 hover:bg-muted/30 focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-ring/50"
    >
      <div className="flex items-start justify-between gap-3">
...
export default async function AdminOverviewPage() {
  ...
  if (!session?.user?.id) {
    redirect(`${SIGN_IN_PATH}?callbackUrl=/dashboard/admin`);
  }
  ...
  const [analytics, recentActivity, totalAuditLogCount, governanceActivityCount] =
    await Promise.all([
      getAdminAnalytics(),
      prisma.auditLog.findMany({
        where: { action: { in: [...recentActivityActions] } },
  ...
  const completionSummary = analytics.completionMonitoring.summary;
  const lockedGoals = getStatusCount(
    analytics.statusDistribution,
    GoalStatus.LOCKED,
  );
  const executiveKpis = buildExecutiveKpis({
    activeEmployeeCount: analytics.activeEmployeeCount,
    approvalRate: analytics.approvalRate,
    approvedGoals: analytics.approvedGoals,
    completionPercentage: analytics.completionPercentage,
    overdueGoals: analytics.overdueGoals,
    overduePercentage: analytics.overduePercentage,
    reviewedGoals: analytics.reviewedGoals,
  ...
  const governanceSnapshot: SnapshotItem[] = [
    {
      ...
      value: analytics.reviewCycle
        ...
        : "None",
      description: analytics.reviewCycle?.label ?? "No active cycle is open.",
      icon: FileClock,
      tone: "bg-sky-50 text-sky-700 ring-sky-200 dark:bg-sky-950/50 dark:text-sky-300 dark:ring-sky-900",
    },
    {
      ...
      description: "Submitted goals awaiting manager review.",
      icon: ClipboardCheck,
      tone: "bg-blue-50 text-blue-700 ring-blue-200 dark:bg-blue-950/50 dark:text-blue-300 dark:ring-blue-900",
    },
    {
      label: "Locked goals",
      value: lockedGoals,
      description: "Active-cycle goals under governance lock.",
      icon: Lock,
      tone: "bg-amber-50 text-amber-700 ring-amber-200 dark:bg-amber-950/50 dark:text-amber-300 dark:ring-amber-900",
    },
    {
      ...
      description: "Recorded approvals, locks, cycle changes, and submissions.",
      icon: ShieldCheck,
      tone: "bg-emerald-50 text-emerald-700 ring-emerald-200 dark:bg-emerald-950/50 dark:text-emerald-300 dark:ring-emerald-900",
    },
    {
      ...
      description: "Audit records available for enterprise review.",
      icon: FileText,
      tone: "bg-slate-100 text-slate-700 ring-slate-200 dark:bg-slate-900/70 dark:text-slate-300 dark:ring-slate-800",
    },
  ];
  const completionSnapshot: SnapshotItem[] = [
    {
      label: "Completed submissions",
      value: completionSummary.completedQuarterlyUpdates,
    ...
    },
    {
      ...
      value: completionSummary.pendingQuarterlyUpdates,
      description: "Approved goals still awaiting current-quarter updates.",
      icon: ListChecks,
      tone: "bg-slate-100 text-slate-700 ring-slate-200 dark:bg-slate-900/70 dark:text-slate-300 dark:ring-slate-800",
    },
    {
      label: "Overdue submissions",
      value: completionSummary.overdueQuarterlyUpdates,
      description: "Submission obligations past the active-cycle deadline.",
      icon: AlertTriangle,
      tone: "bg-amber-50 text-amber-700 ring-amber-200 dark:bg-amber-950/50 dark:text-amber-300 dark:ring-amber-900",
    },
    {
      label: "Awaiting reviews",
      value: completionSummary.pendingReviews,
  ...
  const quickActions: QuickAction[] = [
    {
      label: "Analytics",
      description: "Open the full enterprise analytics dashboard.",
      href: "/dashboard/admin/analytics",
      icon: BarChart3,
    },
    {
      label: "Review cycles",
      description: "Govern active cycle windows and goal locks.",
      href: "/dashboard/admin/review-cycles",
      icon: FileClock,
    },
    {
      label: "Audit logs",
      description: "Review workflow and governance history.",
      href: "/dashboard/admin/audit-logs",
      icon: ShieldCheck,
    },
    {
      label: "Employees",
      description: "Inspect active employees and organization scope.",
      href: "/dashboard/admin/employees",
      icon: UserRound,
    },
    {
      label: "Exports",
      description: "Download CSV or XLSX executive reports.",
      href: "#executive-exports",
      icon: Download,
    },
  ];
  ...
  return (
    <div className="grid gap-6">
          ...
          <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
            <div className="max-w-3xl space-y-3">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm leading-6 text-muted-foreground sm:text-base">
                  Monitor enterprise goal health, approval flow, completion
                  readiness, and governance exposure from one administrator
                  command view.
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-3 rounded-xl border bg-background/80 p-4 shadow-sm sm:grid-cols-3 lg:grid-cols-1">
              ...
              <div>
                ...
                <p className="mt-1 font-semibold">
                  {analytics.submittedGoals} pending approvals
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
                  {lockedGoals} locked goals, {totalAuditLogCount} audit records
                </p>
              </div>
              <div>
                <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
                ...
                <p className="mt-1 font-semibold">
                  {completionSummary.quarterlyCompletionPercentage}% submitted
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
                  {completionSummary.pendingReviews} reviews awaiting decision
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <Card className="rounded-lg">
          <CardHeader className="border-b">
            ...
            <CardDescription>
              Active-cycle controls, pending approval exposure, and audit
              coverage.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
              {governanceSnapshot.map((item) => (
                <SnapshotTile item={item} key={item.label} />
              ))}
            </div>
          </CardContent>
        </Card>
        ...
        <div className="grid gap-6">
          <Card className="rounded-lg">
            <CardHeader className="border-b">
              <CardTitle>Completion visibility</CardTitle>
              <CardDescription>
                Current-quarter submission and review health from completion
                monitoring.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid gap-3 sm:grid-cols-2">
                {completionSnapshot.map((item) => (
                  <SnapshotTile item={item} key={item.label} />
                ))}
              </div>
            </CardContent>
          ...
          <Card className="rounded-lg">
            <CardHeader className="border-b">
              ...
              <CardDescription>
                Navigate to the operating surfaces administrators use most.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid gap-3 sm:grid-cols-2">
        ...
        <CardHeader className="border-b">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <CardDescription>
                Lightweight governance feed for approvals, cycle changes, goal
                locks, and quarterly submissions.
              </CardDescription>
            </div>
        ...
        <CardContent className="p-0">
          {recentActivity.length === 0 ? (
            <div className="px-6 py-12 text-center text-sm text-muted-foreground">
              No recent governance activity has been recorded yet.
            </div>
          ) : (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 line-clamp-1 text-sm text-muted-foreground">
                      {getActivitySubject(log)}
                    </p>
                  </div>
                  <div className="text-left text-xs text-muted-foreground sm:text-right">
                    <p>{formatDateTime(log.createdAt)}</p>
                    <p className="mt-1">{formatActor(log)}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <section id="executive-exports">
        <ExportActions
          actions={executiveExportActions}
