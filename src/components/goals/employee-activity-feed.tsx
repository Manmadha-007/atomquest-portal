...
export type EmployeeActivityFeedItemType =
  | "approval"
  | "feedback"
  | "governance"
  | "goal"
...
type EmployeeActivityFeedProps = {
  items: EmployeeActivityFeedItem[];
};
...
const activityConfig = {
  ...
  EmployeeActivityFeedItemType,
  { icon: LucideIcon; tone: string }
>;

export function EmployeeActivityFeed({ items }: EmployeeActivityFeedProps) {
  return (
    <Card className="rounded-lg">
      <CardHeader className="border-b">
        <CardTitle>Recent execution activity</CardTitle>
        <CardDescription>
          Quarterly submissions, manager decisions, shared-goal events, and
          governance changes.
        </CardDescription>
      </CardHeader>
          ...
          <div className="flex min-h-56 flex-col items-center justify-center gap-3 px-6 text-center">
            ...
            <div className="space-y-1">
              ...
              <p className="max-w-md text-sm text-muted-foreground">
                Execution events will appear after updates, approvals, manager
                feedback, or shared-goal propagation.
              </p>
            </div>
          </div>
        ) : (
